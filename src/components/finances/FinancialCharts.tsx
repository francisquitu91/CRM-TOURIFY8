...
export const FinancialCharts: React.FC<FinancialChartsProps> = ({ transactions, viewMode = 'monthly' }) => {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">
          Progreso hacia la Meta {viewMode === 'monthly' ? 'Mensual' : 'Anual'}
        </h3>
        
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-300">Meta: {formatCurrency(target)}</span>
            <span className="text-sm font-medium text-white">Actual: {formatCurrency(netProfit)}</span>
          </div>
          
          <div className="w-full bg-gray-700 rounded-full h-4">
            <div 
              className={`h-4 rounded-full transition-all duration-500 ${
                netProfit >= target ? 'bg-green-500' : 'bg-indigo-500'
              }`}
              style={{ width: `${Math.min((netProfit / target) * 100, 100)}%` }}
            />
          </div>
          
          <div className="flex justify-between text-xs text-gray-400">
            <span>0%</span>
            <span>{Math.min((netProfit / target) * 100, 100).toFixed(1)}%</span>
            <span>100%</span>
          </div>

          {netProfit >= target && (
            <div className="bg-green-900 border border-green-700 rounded-lg p-4 text-center">
              <p className="text-green-300 font-medium">🎉 ¡Meta alcanzada!</p>
              <p className="text-green-400 text-sm">Has superado el objetivo {viewMode === 'monthly' ? 'mensual' : 'anual'}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl shadow-sm border border-gray-700 p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t border-gray-700">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-white">Utilidad Neta</span>
